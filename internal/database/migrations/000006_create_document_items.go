...
func (m *CreateDocumentItemsTable) Up(tx *gorm.DB) error {
	if err := tx.Migrator().CreateTable(&document.DocumentItem{}); err != nil {
		return err
	}

	// Create indexes for foreign keys
	if !tx.Migrator().HasIndex(&document.DocumentItem{}, "idx_document_items_document_id") {
		if err := tx.Exec("CREATE INDEX IF NOT EXISTS idx_document_items_document_id ON document_items(document_id)").Error; err != nil {
			return err
		}
	}

	if !tx.Migrator().HasIndex(&document.DocumentItem{}, "idx_document_items_item_id") {
...
func (m *CreateDocumentItemsTable) Down(tx *gorm.DB) error {
	return tx.Migrator().DropTable(&document.DocumentItem{})
}
