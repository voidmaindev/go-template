...
func (m *CreateDocumentsTable) Up(tx *gorm.DB) error {
	...
	if err := tx.Migrator().CreateTable(&document.Document{}); err != nil {
		return err
	}

	// Create index for city_id if not already created
	if !tx.Migrator().HasIndex(&document.Document{}, "idx_documents_city_id") {
...
func (m *CreateDocumentsTable) Down(tx *gorm.DB) error {
	return tx.Migrator().DropTable(&document.Document{})
}
